...
import {renderEntity,renderMap,renderTimeline,renderGlossary} from './pages-explore.js';
...
async function route(){const {path,params}=routeState();if(path==='/')return renderHome({main,categories,articles,progress:explorationProgress(),finish});if(path==='/index')return renderIndex({main,categories,articles,params,finish,onRendered});const category=categoryFor(path);if(category){if(category.slug==='tokoh')return renderEntity({main,api,kind:'characters',title:'Tokoh',intro:'Arsip karakter dengan afiliasi, relasi, dan konteks.',finish,onRendered});if(category.slug==='makhluk')return renderEntity({main,api,kind:'creatures',title:'Makhluk Sihir',intro:'Field guide makhluk dengan klasifikasi, habitat, kemampuan, dan risiko dalam lore.',finish,onRendered});if(category.slug==='lokasi')return renderEntity({main,api,kind:'locations',title:'Lokasi Ikonik',intro:'Tempat penting ditautkan dengan peristiwa dan tokoh terkait.',finish,onRendered});return renderIndex({main,categories,articles,category,params,finish,onRendered})}if(path==='/astronomi')return renderEntity({main,api,kind:'astronomy',title:'Astronomi',intro:'Observatorium, langit malam, dan catatan astronomi dalam pengalaman belajar.',finish,onRendered});if(path==='/grimoire/mantra')return renderEntity({main,api,kind:'spells',title:'Mantra',intro:'Indeks mantra sebagai unsur fiksi, lengkap dengan fungsi dan klasifikasi.',finish,onRendered});if(path==='/grimoire/ramuan')return renderEntity({main,api,kind:'potions',title:'Ramuan',intro:'Arsip ramuan dalam lore; bukan instruksi praktik dunia nyata.',finish,onRendered});if(path==='/grimoire/artefak')return renderEntity({main,api,kind:'artifacts',title:'Artefak',intro:'Benda sihir dan perannya dalam peristiwa penting.',finish,onRendered});if(path.startsWith('/artikel/'))return renderArticle({main,categories,articles,slug:path.split('/')[2],api,finish,notice,onRendered,missing});if(path==='/peta')return renderMap({main,finish});if(path==='/timeline')return renderTimeline({main,api,finish,onRendered});if(path==='/relasi')return renderRelationshipGraph({main,finish,onRendered});if(path==='/glosarium')return renderGlossary({main,api,finish,onRendered});if(path==='/bookmark')return renderPersonal({main,articles,categories,kind:'bookmarks',finish});if(path==='/riwayat')return renderPersonal({main,articles,categories,kind:'history',finish});if(path==='/sumber')return renderSources({main,articles,finish});if(path==='/kredit-aset')return renderCredits({main,finish});if(path==='/tentang')return renderAbout({main,finish});return missing()}
